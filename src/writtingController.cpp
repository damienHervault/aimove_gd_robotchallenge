...
int main(int argc, char* argv[]) {
  ...
  ros::init(argc, argv, "writting Controller");
  ros::NodeHandle nh;
  ...
  while (ros::ok()){
    ...
    if (!isnan(pos_wrist[0]) && !isnan(pos_elbow[0])) {
      ...
      unit_wrist_elbow(0) = 0.0;//tableDiff_dir_wrist_elbow[0]/norm_wrist_elbow;
      unit_wrist_elbow(1) = 0.0;//tableDiff_dir_wrist_elbow[1]/norm_wrist_elbow;
      unit_wrist_elbow(2) = -1.0;//tableDiff_dir_wrist_elbow[2]/norm_wrist_elbow;
      ...
      if (first_step){
        ...
        initial_wristZ_pos = wrist_pos[2];
      ...
      wrist_pos_error(2) = 0.0;

      if(wrist_pos[2] - initial_wristZ_pos > 0.2){
        wrist_pos_error(2) =  initial_wristZ_pos + 0.2 - ee_pos[2];
        wrist_up = true;
      }
      if(wrist_pos[2] - initial_wristZ_pos <= 0.2){
        wrist_pos_error(2) = initial_wristZ_pos - ee_pos[2];
        wrist_up = false;
      }
      std::cout<<"wrist_pos_error(2) : "<<wrist_pos_error(2)<<std::endl;
